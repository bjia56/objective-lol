...
func (i *Interpreter) Interpret(ctx context.Context, program *ast.ProgramNode) (environment.Value, error) {
	oldCtx := i.ctx
	i.ctx = ctx
...
func (i *Interpreter) VisitProgram(node *ast.ProgramNode) (environment.Value, error) {
	...
	if mainFunc, err := i.environment.GetFunction("MAIN"); err == nil {
		return i.callFunction(mainFunc, []environment.Value{})
	}
...
func (i *Interpreter) VisitImportStatement(node *ast.ImportStatementNode) (environment.Value, error) {
...
func (i *Interpreter) handleBuiltinImport(moduleName string, declarations []string) (environment.Value, error) {
	...
	if init, ok := i.stdlibInitializer[moduleName]; ok {
		...
		if err != nil {
			return environment.NOTHIN, fmt.Errorf("failed to initialize module %s: %v", moduleName, err)
		}
	} else {
		return environment.NOTHIN, fmt.Errorf("unknown built-in module: %s", moduleName)
	}
...
func (i *Interpreter) handleFileImport(filePath string, declarations []string) (environment.Value, error) {
	// Check for context cancellation
	if err := i.checkContext(); err != nil {
		return environment.NOTHIN, err
	}
	...
	if err != nil {
		return environment.NOTHIN, fmt.Errorf("failed to load module %s: %v", filePath, err)
	}
	...
	if i.moduleResolver.IsInExecutingStack(resolvedPath) {
		return environment.NOTHIN, fmt.Errorf("circular import detected during execution: %s", filePath)
	}
		...
		for _, decl := range moduleAST.Declarations {
			...
			case *ast.FunctionDeclarationNode:
				if _, err := moduleInterpreter.VisitFunctionDeclaration(n); err != nil {
					return environment.NOTHIN, fmt.Errorf("error declaring function in module %s: %v", filePath, err)
				}
			case *ast.ClassDeclarationNode:
				if _, err := moduleInterpreter.VisitClassDeclaration(n); err != nil {
					return environment.NOTHIN, fmt.Errorf("error declaring class in module %s: %v", filePath, err)
				}
			}
		}
		...
		for _, decl := range moduleAST.Declarations {
			...
			case *ast.VariableDeclarationNode:
				if _, err := moduleInterpreter.VisitVariableDeclaration(n); err != nil {
					return environment.NOTHIN, fmt.Errorf("error declaring variable in module %s: %v", filePath, err)
				}
			case *ast.ImportStatementNode:
				if _, err := moduleInterpreter.VisitImportStatement(n); err != nil {
					return environment.NOTHIN, fmt.Errorf("error processing import in module %s: %v", filePath, err)
				}
			}
		}
...
func (i *Interpreter) importSelectiveDeclarations(moduleEnv *environment.Environment, declarations []string, filePath string) (environment.Value, error) {
	for _, declName := range declarations {
		declName = strings.ToUpper(declName)
		if strings.HasPrefix(declName, "_") {
			return environment.NOTHIN, fmt.Errorf("importing private declaration %s from module %s is not allowed", declName, filePath)
		}
		...
		if function, err := moduleEnv.GetFunction(declName); err == nil {
			if err := i.environment.DefineFunction(function); err != nil {
				return environment.NOTHIN, fmt.Errorf("failed to import function %s from module %s: %v", declName, filePath, err)
			}
			continue
		}
		...
		if class, err := moduleEnv.GetClass(declName); err == nil {
			if err := i.environment.DefineClass(class); err != nil {
				return environment.NOTHIN, fmt.Errorf("failed to import class %s from module %s: %v", declName, filePath, err)
			}
			continue
		}
		...
		return environment.NOTHIN, fmt.Errorf("declaration %s not found in module %s", declName, filePath)
	}
...
func (i *Interpreter) importAllDeclarations(moduleEnv *environment.Environment, filePath string) (environment.Value, error) {
	...
	if functions := moduleEnv.GetAllFunctions(); len(functions) > 0 {
		for name, function := range functions {
			...
			if err := i.environment.DefineFunction(function); err != nil {
				return environment.NOTHIN, fmt.Errorf("failed to import function %s from module %s: %v", name, filePath, err)
			}
		}
	}
	...
	if classes := moduleEnv.GetAllClasses(); len(classes) > 0 {
		for name, class := range classes {
			...
			if err := i.environment.DefineClass(class); err != nil {
				return environment.NOTHIN, fmt.Errorf("failed to import class %s from module %s: %v", name, filePath, err)
			}
		}
	}
...
func (i *Interpreter) VisitVariableDeclaration(node *ast.VariableDeclarationNode) (environment.Value, error) {
...
func (i *Interpreter) VisitFunctionDeclaration(node *ast.FunctionDeclarationNode) (environment.Value, error) {
	...
	return environment.NOTHIN, i.environment.DefineFunction(function)
}

// VisitClassDeclaration handles class declarations
func (i *Interpreter) VisitClassDeclaration(node *ast.ClassDeclarationNode) (environment.Value, error) {
	...
	i.currentClass = class.QualifiedName
	...
	return environment.NOTHIN, i.environment.DefineClass(class)
}

// VisitAssignment handles variable assignments
func (i *Interpreter) VisitAssignment(node *ast.AssignmentNode) (environment.Value, error) {
	value, err := node.Value.Accept(i)
	if err != nil {
		return environment.NOTHIN, err
	}
	...
	case *ast.MemberAccessNode:
		// Member assignment (obj member ITZ value)
	...
	default:
		return environment.NOTHIN, fmt.Errorf("invalid assignment target")
	}
}

// VisitIfStatement handles if statements
func (i *Interpreter) VisitIfStatement(node *ast.IfStatementNode) (environment.Value, error) {
...
func (i *Interpreter) VisitReturnStatement(node *ast.ReturnStatementNode) (environment.Value, error) {
	var value environment.Value = environment.NOTHIN
	var err error
...
func (i *Interpreter) VisitFunctionCall(node *ast.FunctionCallNode) (environment.Value, error) {
	// Check for context cancellation
	if err := i.checkContext(); err != nil {
		return environment.NOTHIN, err
	}
	...
	// Evaluate arguments
	args := make([]environment.Value, len(node.Arguments))
	...
	case *ast.IdentifierNode:
		...
		return environment.NOTHIN, fmt.Errorf("undefined function '%s'", functionName)
	...
	case *ast.MemberAccessNode:
		...
		return environment.NOTHIN, fmt.Errorf("cannot call method on non-object")

	default:
		return environment.NOTHIN, fmt.Errorf("invalid function call target")
	}
}
...
func (i *Interpreter) callFunction(function *environment.Function, args []environment.Value) (environment.Value, error) {
	...
	var result environment.Value = environment.NOTHIN
	...
	if function.ReturnType != "" {
		...
		if err != nil {
			return environment.NOTHIN, fmt.Errorf("cannot cast return value to %s: %v", function.ReturnType, err)
		}
		result = castedResult
	}
...
func (i *Interpreter) VisitMemberAccess(node *ast.MemberAccessNode) (environment.Value, error) {
...
func (i *Interpreter) VisitBinaryOp(node *ast.BinaryOpNode) (environment.Value, error) {
	...
	case "MOAR": // Addition
		if leftNum, ok := left.(environment.NumberValue); ok {
			if rightNum, ok := right.(environment.NumberValue); ok {
	...
	case "LES": // Subtraction
		...
		return environment.NOTHIN, fmt.Errorf("operands must be numbers for subtraction")
	...
	case "TIEMZ": // Multiplication
		...
		return environment.NOTHIN, fmt.Errorf("operands must be numbers for multiplication")

	case "DIVIDEZ": // Division
		if leftNum, ok := left.(environment.NumberValue); ok {
			if rightNum, ok := right.(environment.NumberValue); ok {
				// Check for division by zero
				if (rightNum == environment.IntegerValue(0)) || (rightNum == environment.DoubleValue(0.0)) {
					return environment.NOTHIN, runtime.Exception{Message: "Division by zero"}
				}
				return leftNum.Divide(rightNum), nil
			}
		}
		return environment.NOTHIN, runtime.Exception{Message: "Operands must be numbers for division"}
	...
	case "BIGGR THAN": // Greater than
		...
		return environment.NOTHIN, fmt.Errorf("operands must be numbers for comparison")
	...
	case "AN": // Logical AND
		return environment.BoolValue(left.ToBool() == environment.YEZ && right.ToBool() == environment.YEZ), nil

	case "OR": // Logical OR
		return environment.BoolValue(left.ToBool() == environment.YEZ || right.ToBool() == environment.YEZ), nil

	default:
		return environment.NOTHIN, fmt.Errorf("unknown binary operator: %s", node.Operator)
	}
}

// VisitUnaryOp handles unary operations
func (i *Interpreter) VisitUnaryOp(node *ast.UnaryOpNode) (environment.Value, error) {
	...
	switch strings.ToUpper(node.Operator) {
	case "NOT":
		return environment.BoolValue(operand.ToBool() == environment.NO), nil
	default:
		return environment.NOTHIN, fmt.Errorf("unknown unary operator: %s", node.Operator)
	}
}

// VisitCast handles type casting
func (i *Interpreter) VisitCast(node *ast.CastNode) (environment.Value, error) {
	...
	if castErr != nil {
		return environment.NOTHIN, runtime.Exception{Message: castErr.Error()}
	}
	return result, nil
}

// VisitLiteral handles literal values
func (i *Interpreter) VisitLiteral(node *ast.LiteralNode) (environment.Value, error) {
	return node.Value, nil
}

// VisitIdentifier handles identifier references
func (i *Interpreter) VisitIdentifier(node *ast.IdentifierNode) (environment.Value, error) {
	name := strings.ToUpper(node.Name)
	...
	return environment.NOTHIN, runtime.Exception{Message: fmt.Sprintf("Undefined variable or function '%s'", name)}
}
...
func (i *Interpreter) VisitStatementBlock(node *ast.StatementBlockNode) (environment.Value, error) {
	var result environment.Value = environment.NOTHIN
...
func (i *Interpreter) VisitTryStatement(node *ast.TryStatementNode) (environment.Value, error) {
	// Check for context cancellation
	if err := i.checkContext(); err != nil {
		return environment.NOTHIN, err
	}
	...
	if node.FinallyBody != nil {
		...
		if finallyErr != nil {
			return environment.NOTHIN, finallyErr
		}
	}
...
func (i *Interpreter) VisitThrowStatement(node *ast.ThrowStatementNode) (environment.Value, error) {
	...
	if err != nil {
		return environment.NOTHIN, fmt.Errorf("exception message must be a string, got %s", msgValue.Type())
	}

	// Create and throw the exception
	exceptionMsg := strValue.String()
	return environment.NOTHIN, runtime.Exception{Message: exceptionMsg}
}
