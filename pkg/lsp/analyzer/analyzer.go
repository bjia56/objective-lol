...
func (a *Analyzer) AnalyzeDocument(uri, content string) ([]protocol.Diagnostic, error) {
	// Create or get cached semantic analyzer
...
func (a *Analyzer) GetHoverInfo(uri, content string, position protocol.Position) (*protocol.Hover, error) {
	...
	if _, exists := a.semanticCache[uri]; !exists {
		if _, err := a.AnalyzeDocument(uri, content); err != nil {
			return nil, err
		}
	}

	semanticAnalyzer := a.semanticCache[uri]
